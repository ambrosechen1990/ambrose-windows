...
class MainApplication:
    ...
    def create_function_areas(self):
        functions = [
            ...
            {"name": "文件解析", "command": self.unzip_and_parse_zip, "row": 0, "column": 1, "icon": resource_path("icons/文件解析.jpeg")},
    ...
    def extract_zip_and_parse_with_progress(self, archive_path):
        ...
        extract_dir = os.path.splitext(os.path.splitext(archive_path)[0])[0] if archive_path.endswith('.tar.gz') else os.path.splitext(archive_path)[0]
        ...
        count = 0
        for i, args in enumerate(bin_files, 1):
            process_one_bin(args)
            count += 1
